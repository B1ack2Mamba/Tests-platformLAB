...
export default function ProjectResultsStandalonePage() {
  ...
  }, [blueprint, data, focusInput, roleInput]);
  ...
  if (loading || busy || (!data && !error)) {
    return (
      <Layout title="Страница результатов">
        <div className="mx-auto max-w-[1240px] px-3 pb-10 pt-3 sm:px-4">
          <div className="rounded-[30px] border border-[#dcc8aa] bg-[linear-gradient(180deg,#fffdfa_0%,#f6efe4_100%)] p-5 shadow-[0_24px_54px_rgba(93,71,39,0.10)]">
            ...
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              <div className="h-24 animate-pulse rounded-[24px] bg-[#f4ecde]" />
              <div className="h-24 animate-pulse rounded-[24px] bg-[#f4ecde]" />
            </div>
            <div className="mt-6 h-[520px] animate-pulse rounded-[24px] bg-[#f4ecde]" />
          </div>
        </div>
      </Layout>
    );
  ...
  return (
    <Layout title={data?.project.title ? `${data.project.title} — результаты` : "Страница результатов"}>
      <div className="mx-auto max-w-[1240px] px-3 pb-12 pt-3 sm:px-4">
        {error ? <div className="mb-4 rounded-[20px] border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{error}</div> : null}

        <div className="rounded-[34px] border border-[#dcc8aa] bg-[linear-gradient(180deg,#fffdfa_0%,#f5eee2_100%)] p-4 shadow-[0_26px_60px_rgba(93,71,39,0.12)] sm:p-5 lg:p-6">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
              <div className="text-[11px] uppercase tracking-[0.24em] text-[#9d7a4b]">Отдельная страница результатов</div>
              <h1 className="mt-2 text-[1.7rem] font-semibold leading-tight text-[#2d2a22] sm:text-[2.05rem]">
                {data?.project.title || "Проект"}
              </h1>
              <div className="mt-2 flex flex-wrap gap-2 text-xs text-[#7b664f]">
                ...
                {blueprint ? <span className="rounded-full border border-[#e3d4bd] bg-white/70 px-3 py-1">{blueprint.summary.routeMode === "competency" ? "Контур компетенций" : "Контур цели"}</span> : null}
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Link href={`/projects/${projectId}`} className="rounded-2xl border border-[#d9c4a4] bg-[#fffaf0] px-4 py-2 text-sm font-medium text-[#5b4731] shadow-[0_8px_18px_rgba(93,71,39,0.08)]">
                Назад к проекту
              </Link>
              {isAdminEmail(user.email) ? (
                <Link href="/admin/competency-prompts" className="rounded-2xl border border-[#d9c4a4] bg-[#fffaf0] px-4 py-2 text-sm font-medium text-[#5b4731] shadow-[0_8px_18px_rgba(93,71,39,0.08)]">
          ...
          {!data?.fully_done || !blueprint ? (
            <div className="mt-6 rounded-[26px] border border-[#d8c5a8] bg-[#fbf5ea] px-5 py-4 text-sm leading-7 text-[#6f6454] shadow-[0_16px_34px_rgba(93,71,39,0.08)]">
              Эта страница откроется после завершения всех тестов в проекте. Сейчас готово {data?.completed || 0} из {data?.total || 0}.
            </div>
          ) : (
            <>
              <div className="mt-6 grid gap-4 lg:grid-cols-[minmax(0,1fr)_340px]">
                <section className="rounded-[28px] border border-[#d8c5a8] bg-white/68 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.76)] sm:p-5">
                  <div className="text-sm font-semibold text-[#2d2a22]">Внешний слой</div>
                  <div className="mt-1 text-sm text-[#8b7760]">Минимум управления снаружи, а внутреннюю логику можно открыть только при необходимости.</div>
                  ...
                  <div className="mt-4 grid gap-3">
                    <div className="rounded-[24px] border border-[#dfcfb5] bg-[#fffaf1] px-4 py-3.5">
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs text-[#7b664f]">
                    <span className="rounded-full border border-[#e3d4bd] bg-white/70 px-3 py-1">{promptCoverageLine(blueprint)}</span>
                    <span className="rounded-full border border-[#e3d4bd] bg-white/70 px-3 py-1">Фокус: {blueprint.summary.focusLabel}</span>
                  </div>
                </section>

                <aside className="rounded-[28px] border border-[#d8c5a8] bg-[linear-gradient(180deg,#fffaf2_0%,#f8f1e7_100%)] p-4 shadow-[0_18px_38px_rgba(93,71,39,0.10)] sm:p-5">
                  <div className="text-sm font-semibold text-[#2d2a22]">Короткий статус карты</div>
                  <div className="mt-4 space-y-3 text-sm leading-7 text-[#5f5446]">
                    ...
                    <div className="rounded-[22px] border border-[#e2d3bb] bg-white/65 px-4 py-3">
                      <div className="text-[11px] uppercase tracking-[0.18em] text-[#9d7a4b]">Механика</div>
                      <div className="mt-2">{blueprint.summary.finalText}</div>
                    </div>
                  </div>
              ...
              {showMechanism ? (
                <div className="mt-6 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px] xl:items-start">
                  ...
                  <aside className="rounded-[30px] border border-[#d8c5a8] bg-[linear-gradient(180deg,#fffaf2_0%,#f7efe3_100%)] p-5 shadow-[0_20px_42px_rgba(93,71,39,0.10)]">
