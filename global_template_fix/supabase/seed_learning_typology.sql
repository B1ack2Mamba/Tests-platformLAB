-- Seed: learning-typology (Типология личности обучения)
-- 1) Run supabase/schema.sql + supabase/training_rooms.sql first.
-- 2) Then run this seed.

begin;

insert into public.tests (slug, title, description, type, json, price_rub, is_published)
values (
  'learning-typology',
  'Типология личности обучения',
  'Опросник на определение преобладающего стиля обучения: практик, теоретик, наблюдатель или экспериментатор.',
  'learning_typology_v1',
  $$
{
  "slug": "learning-typology",
  "title": "Типология личности обучения",
  "description": "Опросник на определение преобладающего стиля обучения: практик, теоретик, наблюдатель или экспериментатор.",
  "instructions": "Выберите по одному варианту в каждом пункте. Вопросы показывают, какой стиль обучения и освоения опыта выражен у вас сильнее.",
  "type": "learning_typology_v1",
  "pricing": {
    "interpretation_rub": 0,
    "details_rub": 49
  },
  "has_interpretation": false,
  "questions": [
    {
      "order": 1,
      "text": "Вы пришли в гости, где собралось уже более 10 человек. Ваша реакция:",
      "options": [
        {
          "code": "A",
          "text": "Здорово! Обожаю шумные компании, можно повеселиться, завести новых знакомых.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Я люблю бывать в компаниях, часто оказываюсь в центре внимания. Или удастся хорошенько зажечь, или, на худой конец, с полезными людьми познакомлюсь.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "C",
          "text": "Надеюсь, что встречу здесь каких-нибудь знакомых, мне будет приятно с ними пообщаться. Если все — незнакомые люди, то мне будет некомфортно.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Я не очень люблю шумные компании и хожу на вечеринки только чтобы завести или поддержать полезные знакомства. Мне приятнее посидеть и поговорить с одним-двумя людьми в покое и тишине.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 2,
      "text": "На этой же вечеринке вас попросили произнести тост. Ваша реакция:",
      "options": [
        {
          "code": "A",
          "text": "Я не люблю привлекать к себе внимание, ненавижу произносить тосты. Я не буду ни соглашаться, ни отказываться, а просто как-нибудь увильну.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "B",
          "text": "Я хороший рассказчик и знаю пару прикольных тостов. Все будут в восторге.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "C",
          "text": "Я не боюсь произносить тосты, даже получаю от этого удовольствие, скажу что-нибудь умное и по делу.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "D",
          "text": "Я, скорее всего, откажусь под убедительным предлогом. Но если мне надо произвести хорошее впечатление для пользы дела, то могу и произнести уместный изящный тост.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 3,
      "text": "Ваш начальник дал вам и вашему сослуживцу задание, но не назначил ответственного за его выполнение. Вы оба понадеялись друг на друга и забыли про задание. Сейчас ваш начальник ругает вас за невыполнение задания. Ваша реакция:",
      "options": [
        {
          "code": "A",
          "text": "Негативные эмоции, которые никак не проявляются внешне. Я в состоянии контролировать себя, но теперь я знаю, чего можно ожидать от этих людей, и буду осторожнее в работе с ними в будущем.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "B",
          "text": "Очень эмоциональная реакция, возможны слёзы. Ну да, я — рассеянный и не очень пунктуальный человек, но не я один(одна) виноват(а). Буду жаловаться на них всем друзьям и знакомым.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "C",
          "text": "Обида на сослуживца. Как он мог так подставить меня! Обида на начальника. Я буду долго переживать из-за этого, скорее всего, молча.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Злость на начальника, потому что он безграмотный руководитель, и/или на сослуживца, потому что он меня подставил. Всплеск агрессивных эмоций. Скорее всего, кому-нибудь из них что-нибудь выскажу.",
          "tags": [
            "PRA"
          ]
        }
      ]
    },
    {
      "order": 4,
      "text": "Вам дали важное задание. Срок выполнения — через месяц, а выполнить его можно за две недели. Ваша реакция:",
      "options": [
        {
          "code": "A",
          "text": "Лучше поскорее выполню задание и сдам его. И в глазах начальства буду хорошо выглядеть, и у меня время для других дел будет.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Сначала мне надо подумать, как лучше всего подойти к выполнению этой работы. Даже если сделаю её заранее, сразу сдавать не буду. Пусть отлежится, потом ещё раз исправлю ошибки. Может, сдам работу за день до срока.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "C",
          "text": "Сразу возьмусь за дело. Но, скорее всего, мне быстро надоест эта работа, и я увлекусь другим заданием. Заброшу эту работу до тех пор, пока не наступит крайний срок. Потом срочно буду всё доделывать, может, даже опоздаю со сдачей.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "D",
          "text": "Хотелось бы начать делать сразу, но знаю, что так не получится. Всегда есть более срочные или важные дела, всё время что-то отвлекает. Скорее всего, буду выполнять задание до последней минуты.",
          "tags": [
            "OBS"
          ]
        }
      ]
    },
    {
      "order": 5,
      "text": "Впереди длинные выходные. Вы решили куда-нибудь выбраться. Что вы выберете:",
      "options": [
        {
          "code": "A",
          "text": "Съездить в гости к родственникам или иначе провести день с семьёй, супругом(ой).",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "B",
          "text": "Сходить в парк или на вечеринку с друзьями.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "C",
          "text": "Съездить на картинг или поиграть в футбол (карты) с друзьями.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "D",
          "text": "Сходить на концерт или выставку, можно в одиночку или с близким человеком.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 6,
      "text": "Если бы вы решили прыгать с парашютом, то по какой причине:",
      "options": [
        {
          "code": "A",
          "text": "Я познакомился с очень интересными людьми (человеком), которые занимаются парашютным спортом. Они меня уговорили присоединиться.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Мне необходимо через это пройти для достижения важной для меня цели.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "C",
          "text": "Я вообще люблю риск, адреналин. Хочу узнать, на что я способен.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "D",
          "text": "Меня считают тихоней. Я всегда в тени, отмалчиваюсь. Хочу доказать себе и другим, что я не трус и не тряпка.",
          "tags": [
            "OBS"
          ]
        }
      ]
    },
    {
      "order": 7,
      "text": "Какие комментарии вы слышите чаще (как на работе, так и дома) в свой адрес:",
      "options": [
        {
          "code": "A",
          "text": "«А быстрее нельзя?» «Ты опять тянешь время!» «Сколько можно обсуждать одно и то же».",
          "tags": [
            "OBS",
            "THE"
          ]
        },
        {
          "code": "B",
          "text": "«Пожалуйста, помедленнее». «Ты опять всех торопишь, мы никуда не опаздываем!» «Тебе уже всё ясно, а мне — ещё нет. Давай не торопясь всё обсудим».",
          "tags": [
            "EXP",
            "PRA"
          ]
        }
      ]
    },
    {
      "order": 8,
      "text": "Вы узнали, что вас решили повысить по службе. Что вы сделаете прежде всего:",
      "options": [
        {
          "code": "A",
          "text": "Сообщите родным и близким о повышении, устроите уютный домашний праздник.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "B",
          "text": "Купите себе какую-нибудь дорогую вещь, чтобы в достойном виде прибыть на место работы в первый день на новой должности (часы, костюм, машину).",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "C",
          "text": "Поделитесь радостью с друзьями, закатите большую шумную вечеринку.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "D",
          "text": "Подождёте радоваться, сорить деньгами и трезвонить о повышении, пока не начнёте работать на новом месте. Ведь ещё даже приказ не подписан.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 9,
      "text": "У вас завтра экзамен. Ваше поведение:",
      "options": [
        {
          "code": "A",
          "text": "Лучше быстро повторить материал, чтобы осталось время на другие важные для вас дела.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Лучше ещё раз не спеша всё повторить, пускай это даже займёт всю ночь.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "C",
          "text": "Лучше хорошенько отоспаться перед экзаменом, чтобы прийти на него со свежей головой. К экзамену вы готовились заранее.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "D",
          "text": "Перед смертью не надышишься. Лучше вообще не думать об экзамене, а пойти повеселиться.",
          "tags": [
            "EXP"
          ]
        }
      ]
    },
    {
      "order": 10,
      "text": "Как вы считаете, что является главным для победы, для достижения успеха:",
      "options": [
        {
          "code": "A",
          "text": "Личные усилия каждого. Каждый человек должен отвечать за себя, выкладываться по максимуму, не прятаться за спинами других.",
          "tags": [
            "PRA",
            "THE"
          ]
        },
        {
          "code": "B",
          "text": "Главное — командная работа, люди могут добиться чего-либо только вместе, помогая и поддерживая друг друга.",
          "tags": [
            "OBS",
            "EXP"
          ]
        }
      ]
    },
    {
      "order": 11,
      "text": "Если бы вы решили открыть свой бизнес (успех и одинаковая прибыльность гарантированы), что бы вы выбрали:",
      "options": [
        {
          "code": "A",
          "text": "Фирму финансового консалтинга или фирму по борьбе с вредными насекомыми.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "B",
          "text": "Охранную фирму или оружейный магазин.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "C",
          "text": "Ресторан или ночной клуб.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "D",
          "text": "Медицинский центр или бюро добрых услуг.",
          "tags": [
            "OBS"
          ]
        }
      ]
    },
    {
      "order": 12,
      "text": "У вас — новый просторный кабинет. Чем вы украсите его стены:",
      "options": [
        {
          "code": "A",
          "text": "Своими фото с известными людьми или яркими современными картинами.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Фото супруга(и) и детей или групповыми фото ваших сослуживцев на корпоративе.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "C",
          "text": "Дипломами или нейтральными картинами.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "D",
          "text": "Портретом президента или старинной саблей.",
          "tags": [
            "PRA"
          ]
        }
      ]
    },
    {
      "order": 13,
      "text": "Что вы цените более всего в одежде?",
      "options": [
        {
          "code": "A",
          "text": "В одежде обязательно должна быть какая-нибудь изюминка, кураж.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Одежда должна выглядеть дорого и круто.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "C",
          "text": "Одежда должна быть удобной.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Одежда должна быть качественной и уместной, то есть соответствовать возрасту, фигуре и ситуации, в которой её надеваешь.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 14,
      "text": "Вам надо выбрать соревнование, в котором вам комфортнее всего будет участвовать и где у вас больше шансов на победу. Что вы выберете?",
      "options": [
        {
          "code": "A",
          "text": "Индивидуальные соревнования, где оценивается сообразительность, а не быстрота реакции (шахматы, бильярд, покер).",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "B",
          "text": "Индивидуальные соревнования на скорость и смелость (прыжки с парашютом, автогонки, горные лыжи).",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "C",
          "text": "Командные соревнования, лучше какие-нибудь необычные (футбол в грязи, корпоративные весёлые старты).",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "D",
          "text": "Командные соревнования, где требуется взаимная поддержка, взаимодействие всей команды (соревнование университетов по созданию лучшего робота, кёрлинг).",
          "tags": [
            "OBS"
          ]
        }
      ]
    },
    {
      "order": 15,
      "text": "Вам надо выбрать гостиницу в другом городе. Что вы выберете?",
      "options": [
        {
          "code": "A",
          "text": "Какую-нибудь приличную гостиницу в центре, чтобы не стыдно было.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Какой-нибудь прикольный необычный мини-отель.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "C",
          "text": "Гостиницу, где раньше останавливался(ась), или ту, которую порекомендуют знакомые.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Гостиницу с идеальным соотношением цена/качество. Может быть, в старинном доме в ретростиле.",
          "tags": [
            "THE"
          ]
        }
      ]
    },
    {
      "order": 16,
      "text": "Вы лучше усвоите новый материал, если:",
      "options": [
        {
          "code": "A",
          "text": "Существует очевидная связь между изучаемым предметом и проблемами, решаемыми на работе.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Объект изучения даётся в контексте систем, концепций, теорий.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "C",
          "text": "Существует возможность для тщательного обдумывания и наблюдения.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Существуют новые ситуации, возможности, проблемы, на которых можно учиться.",
          "tags": [
            "EXP"
          ]
        }
      ]
    },
    {
      "order": 17,
      "text": "Мотивация на обучение значительно снижается, если:",
      "options": [
        {
          "code": "A",
          "text": "Обучение не ведёт к явному улучшению, например к увеличению объёма продаж или экономии времени.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Изучаемый предмет представляется банальным, поверхностным.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "C",
          "text": "Существуют ограничения по времени и необходимо быстро переключаться с одного вида деятельности на другой.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "D",
          "text": "Необходимо следовать точным инструкциям, оставляющим малое пространство для манёвра и инициативы.",
          "tags": [
            "EXP"
          ]
        }
      ]
    },
    {
      "order": 18,
      "text": "Вы считаете, что при выполнении задачи вашей ахиллесовой пятой является:",
      "options": [
        {
          "code": "A",
          "text": "Сначала действия, потом оценка последствий.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Затягивание решений, неготовность рисковать.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "C",
          "text": "Излишняя схематизация.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "D",
          "text": "Ориентация преимущественно на задачу, а не на людей.",
          "tags": [
            "PRA"
          ]
        }
      ]
    },
    {
      "order": 19,
      "text": "В принятии решения и работе над поставленной задачей вы всегда:",
      "options": [
        {
          "code": "A",
          "text": "Готовы к новой деятельности и риску.",
          "tags": [
            "EXP"
          ]
        },
        {
          "code": "B",
          "text": "Выслушиваете другие точки зрения и склонны не делать поспешных выводов.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "C",
          "text": "Систематизируете и задаёте вопросы, проясняющие суть.",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "D",
          "text": "Ориентируетесь на практичность и реалистичность.",
          "tags": [
            "PRA"
          ]
        }
      ]
    },
    {
      "order": 20,
      "text": "Ваш девиз по жизни:",
      "options": [
        {
          "code": "A",
          "text": "Всё в жизни нужно попробовать.",
          "tags": [
            "PRA"
          ]
        },
        {
          "code": "B",
          "text": "Сначала я должен(должна) это обдумать.",
          "tags": [
            "OBS"
          ]
        },
        {
          "code": "C",
          "text": "Посмотрим, как это согласуется с ...",
          "tags": [
            "THE"
          ]
        },
        {
          "code": "D",
          "text": "Должен существовать способ получше.",
          "tags": [
            "EXP"
          ]
        }
      ]
    }
  ],
  "scoring": {
    "tags": [
      "OBS",
      "EXP",
      "PRA",
      "THE"
    ],
    "tag_to_name": {
      "OBS": "Наблюдатель",
      "EXP": "Экспериментатор",
      "PRA": "Практик",
      "THE": "Теоретик"
    },
    "thresholds_count": {
      "dominant_min": 9,
      "high_min": 7,
      "medium_min": 5
    },
    "blend_close_delta": 1
  }
}
  $$::jsonb,
  0,
  true
)
on conflict (slug) do update set
  title = excluded.title,
  description = excluded.description,
  type = excluded.type,
  json = excluded.json,
  price_rub = excluded.price_rub,
  is_published = excluded.is_published;

commit;
