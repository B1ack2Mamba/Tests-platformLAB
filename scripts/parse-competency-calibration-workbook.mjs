...
const WORKBOOK_PATH = path.join(process.cwd(), "competency_calibration_workbook.xlsx");
const OUTPUT_PATH = path.join(process.cwd(), "data", "competency-calibration", "workbook.json");
...
async function parseWorkbookSheetTargets() {
  ...
  for (const rel of relsXml.matchAll(/<Relationship\b([^>]*)\/>/g)) {
    ...
    if (attrs.Id && attrs.Target) relTargets.set(attrs.Id, `xl/${attrs.Target}`);
  }
  const sheets = [];
