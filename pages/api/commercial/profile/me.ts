...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    logApiError("commercial.profile.me", requestId, error);
    return res.status(400).json({ ok: false, request_id: requestId, error: error?.message || "Не удалось загрузить профиль" });
  }
}
