...
function rowToBenchmark(row: any): ManualCalibrationBenchmark {
  return {
    ...
    registryCommentOverride: row.registry_comment_override || null,
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      const analysis = await buildCandidateRegistryAnalysis({
        project: {
          ...
          registry_comment: (data as any)?.registry_comment_override || project.registry_comment || null,
    ...
    const payload = {
      ...
      registry_comment_override: String(body.registry_comment_override || "").trim() || null,
