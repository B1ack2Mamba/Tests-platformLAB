...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const sceneWidgets = sanitizeSceneWidgetTemplates(body.scene_widgets);
  ...
  return res.status(200).json({
    ok: true,
    template: data ? { ...data, scene_widgets: sanitizeSceneWidgetTemplates((data as any).scene_widgets) } : data,
  });
}
