-- Seed: usk (Уровень субъективного контроля)
-- 1) Run supabase/schema.sql + supabase/paywall.sql + supabase/wallet_debit.sql + supabase/yookassa.sql + supabase/training_rooms.sql first.
-- 2) Then run this seed.

begin;

insert into public.tests (slug, title, description, type, json, price_rub, is_published)
values (
  'usk',
  'Уровень субъективного контроля (УСК)',
  'Опросник локуса контроля. Шкала ответов от −3 до +3. Итоги по 7 шкалам интернальности (стены 1–10).',
  'usk_v1',
  $$
{
  "slug": "usk",
  "title": "Уровень субъективного контроля (УСК)",
  "description": "Опросник локуса контроля. Шкала ответов от −3 до +3. Итоги по 7 шкалам интернальности (стены 1–10).",
  "type": "usk_v1",
  "pricing": {
    "interpretation_rub": 0,
    "details_rub": 49
  },
  "has_interpretation": false,
  "questions": [
    {
      "order": 1,
      "text": "Продвижение по службе больше зависит от удачного стечения обстоятельств, чем от способностей и усилий человека."
    },
    {
      "order": 2,
      "text": "Большинство разводов происходит от того, что люди не захотели приспособиться друг к другу."
    },
    {
      "order": 3,
      "text": "Болезнь – дело случая; если уж суждено заболеть, то ничего не поделаешь."
    },
    {
      "order": 4,
      "text": "Люди оказываются одинокими из-за того, что сами не проявляют интереса и дружелюбия к окружающим."
    },
    {
      "order": 5,
      "text": "Осуществление моих желаний часто зависит от везения."
    },
    {
      "order": 6,
      "text": "Бесполезно предпринимать усилия для того, чтобы завоевать симпатии других людей."
    },
    {
      "order": 7,
      "text": "Внешние обстоятельства - родители и благосостояние - влияют на семейное счастье не меньше, чем отношения супругов"
    },
    {
      "order": 8,
      "text": "Я часто чувствую, что мало влияю на то, что происходит со мной."
    },
    {
      "order": 9,
      "text": "Как правило, руководство оказывается более эффективным, когда полностью контролирует действия подчиненных, а не полагается на их самостоятельность."
    },
    {
      "order": 10,
      "text": "Мои отметки в школе часто зависели от случайных обстоятельств (например, настроения учителя), чем от моих собственных усилий."
    },
    {
      "order": 11,
      "text": "Когда я строю планы, то я, в общем, верю, что смогу осуществить их."
    },
    {
      "order": 12,
      "text": "То, что многим людям кажется удачей или везением, на самом деле является результатом долгих целенаправленных усилий"
    },
    {
      "order": 13,
      "text": "Думаю, что правильный образ жизни может больше помочь здоровью, чем врачи и лекарства."
    },
    {
      "order": 14,
      "text": "Если люди не подходят друг к другу, то, как бы они ни старались, наладить семейную жизнь, они все равно не смогут."
    },
    {
      "order": 15,
      "text": "То хорошее, что я делаю, обычно бывает по достоинству оценено другими."
    },
    {
      "order": 16,
      "text": "Дети вырастают такими, какими их воспитывают родители."
    },
    {
      "order": 17,
      "text": "Думаю, что случай или судьба не играют важной роли в моей жизни."
    },
    {
      "order": 18,
      "text": "Я стараюсь не планировать далеко вперед, потому что немало е зависит от того, как сложатся обстоятельства."
    },
    {
      "order": 19,
      "text": "Мои отметки в школе больше всего зависели от моих усилий и степени подготовленности."
    },
    {
      "order": 20,
      "text": "В семейных конфликтах я чаще чувствую вину за собой, чем за противоположной стороной."
    },
    {
      "order": 21,
      "text": "Жизнь большинства людей зависит от стечения обстоятельств."
    },
    {
      "order": 22,
      "text": "Я предпочитаю такое руководство, при котором можно самостоятельно определять, как и что делать."
    },
    {
      "order": 23,
      "text": "Думаю, что мой образ жизни ни в коей мере не является причиной моих болезней."
    },
    {
      "order": 24,
      "text": "Как правило, именно неудачное стечение обстоятельств мешает людям добиться успехов в своем деле."
    },
    {
      "order": 25,
      "text": "В конце концов, за плохое управление организацией ответственны сами люди, которые в ней работают."
    },
    {
      "order": 26,
      "text": "Я часто чувствую, что ничего не могу изменить в сложившихся отношениях в семье."
    },
    {
      "order": 27,
      "text": "Если я очень захочу, то смогу расположить к себе почти любого."
    },
    {
      "order": 28,
      "text": "На подрастающее поколение влияет так немало разных обстоятельств, что усилия родителей по их воспитанию часто оказываются бесполезными."
    },
    {
      "order": 29,
      "text": "То, что со мной случится, это дело моих собственных рук."
    },
    {
      "order": 30,
      "text": "Трудно бывает понять, почему руководители поступают так, а не иначе."
    },
    {
      "order": 31,
      "text": "Человек, который не смог добиться успеха в своей работе, скорее всего не проявлял достаточно усилий."
    },
    {
      "order": 32,
      "text": "Чаще всего я могу добиться от членов моей семьи того, что я хочу."
    },
    {
      "order": 33,
      "text": "В неприятностях и неудачах, которые были в моей жизни , чаще всего были виноваты другие люди, чем я сам."
    },
    {
      "order": 34,
      "text": "Ребенка всегда можно уберечь от простуды, если за ним следить и правильно его одевать."
    },
    {
      "order": 35,
      "text": "В сложных обстоятельствах я предпочитаю подождать, пока проблемы разрешатся сами собой."
    },
    {
      "order": 36,
      "text": "Успех является результатом упорной работы и мало зависит от случая или везения."
    },
    {
      "order": 37,
      "text": "Я чувствую, что от меня больше, чем от кого бы то ни было, зависит счастье моей семьи."
    },
    {
      "order": 38,
      "text": "Мне всегда было трудно понять, почему я нравлюсь одним людям и не нравлюсь другим."
    },
    {
      "order": 39,
      "text": "Я всегда предпочитаю принимать решение и действовать самостоятельно, а не надеяться на помощь других людей или на судьбу."
    },
    {
      "order": 40,
      "text": "К сожалению, заслуги человека часто остаются непризнанными , несмотря на все его старания."
    },
    {
      "order": 41,
      "text": "В семейной жизни бывают такие ситуации, которые невозможно разрешить даже при самом сильном желании."
    },
    {
      "order": 42,
      "text": "Способные люди, не сумевшие реализовать свои возможности, должны винить в этом только самих себя."
    },
    {
      "order": 43,
      "text": "Многие мои успехи были возможны только благодаря помощи других людей."
    },
    {
      "order": 44,
      "text": "Большинство неудач в моей жизни произошло от неумения, незнания или лени и мало зависело от везения или невезения."
    }
  ],
  "scoring": {
    "scales": [
      "IO",
      "ID",
      "IN",
      "IS",
      "IP",
      "IM",
      "IZ"
    ],
    "scale_to_name": {
      "IO": "Общая интернальность (Ио)",
      "ID": "Интернальность достижений (Ид)",
      "IN": "Интернальность неудач (Ин)",
      "IS": "Интернальность семейных отношений (Ис)",
      "IP": "Интернальность производственных отношений (Ип)",
      "IM": "Интернальность межличностных отношений (Им)",
      "IZ": "Интернальность здоровья и болезни (Из)"
    },
    "keys": {
      "IO": {
        "plus": [
          2,
          4,
          11,
          12,
          13,
          15,
          16,
          17,
          19,
          20,
          22,
          25,
          27,
          29,
          31,
          32,
          34,
          36,
          37,
          39,
          42,
          44
        ],
        "minus": [
          1,
          3,
          5,
          6,
          7,
          8,
          9,
          10,
          14,
          18,
          21,
          23,
          24,
          26,
          28,
          30,
          33,
          35,
          38,
          40,
          41,
          43
        ]
      },
      "ID": {
        "plus": [
          12,
          15,
          27,
          32,
          36,
          37
        ],
        "minus": [
          1,
          5,
          6,
          14,
          26,
          43
        ]
      },
      "IN": {
        "plus": [
          2,
          4,
          20,
          31,
          42,
          44
        ],
        "minus": [
          7,
          24,
          33,
          38,
          40,
          41
        ]
      },
      "IS": {
        "plus": [
          2,
          16,
          20,
          32,
          37
        ],
        "minus": [
          7,
          14,
          26,
          28,
          41
        ]
      },
      "IP": {
        "plus": [
          19,
          22,
          25,
          31,
          42
        ],
        "minus": [
          1,
          9,
          10,
          24,
          30
        ]
      },
      "IM": {
        "plus": [
          4,
          27
        ],
        "minus": [
          6,
          38
        ]
      },
      "IZ": {
        "plus": [
          13,
          34
        ],
        "minus": [
          3,
          23
        ]
      }
    },
    "stens": {
      "IO": [
        {
          "min": -132,
          "max": -14,
          "sten": 1
        },
        {
          "min": -13,
          "max": -3,
          "sten": 2
        },
        {
          "min": -2,
          "max": 9,
          "sten": 3
        },
        {
          "min": 10,
          "max": 21,
          "sten": 4
        },
        {
          "min": 22,
          "max": 32,
          "sten": 5
        },
        {
          "min": 33,
          "max": 44,
          "sten": 6
        },
        {
          "min": 45,
          "max": 56,
          "sten": 7
        },
        {
          "min": 57,
          "max": 68,
          "sten": 8
        },
        {
          "min": 69,
          "max": 79,
          "sten": 9
        },
        {
          "min": 80,
          "max": 132,
          "sten": 10
        }
      ],
      "ID": [
        {
          "min": -36,
          "max": -11,
          "sten": 1
        },
        {
          "min": -10,
          "max": -7,
          "sten": 2
        },
        {
          "min": -6,
          "max": -3,
          "sten": 3
        },
        {
          "min": -2,
          "max": 1,
          "sten": 4
        },
        {
          "min": 2,
          "max": 5,
          "sten": 5
        },
        {
          "min": 6,
          "max": 9,
          "sten": 6
        },
        {
          "min": 10,
          "max": 14,
          "sten": 7
        },
        {
          "min": 15,
          "max": 18,
          "sten": 8
        },
        {
          "min": 19,
          "max": 22,
          "sten": 9
        },
        {
          "min": 23,
          "max": 36,
          "sten": 10
        }
      ],
      "IN": [
        {
          "min": -36,
          "max": -8,
          "sten": 1
        },
        {
          "min": -7,
          "max": -4,
          "sten": 2
        },
        {
          "min": -3,
          "max": 0,
          "sten": 3
        },
        {
          "min": 1,
          "max": 4,
          "sten": 4
        },
        {
          "min": 5,
          "max": 7,
          "sten": 5
        },
        {
          "min": 8,
          "max": 11,
          "sten": 6
        },
        {
          "min": 12,
          "max": 15,
          "sten": 7
        },
        {
          "min": 16,
          "max": 19,
          "sten": 8
        },
        {
          "min": 20,
          "max": 23,
          "sten": 9
        },
        {
          "min": 24,
          "max": 36,
          "sten": 10
        }
      ],
      "IS": [
        {
          "min": -30,
          "max": -12,
          "sten": 1
        },
        {
          "min": -11,
          "max": -8,
          "sten": 2
        },
        {
          "min": -7,
          "max": -5,
          "sten": 3
        },
        {
          "min": -4,
          "max": -1,
          "sten": 4
        },
        {
          "min": 0,
          "max": 3,
          "sten": 5
        },
        {
          "min": 4,
          "max": 6,
          "sten": 6
        },
        {
          "min": 7,
          "max": 10,
          "sten": 7
        },
        {
          "min": 11,
          "max": 13,
          "sten": 8
        },
        {
          "min": 14,
          "max": 17,
          "sten": 9
        },
        {
          "min": 18,
          "max": 30,
          "sten": 10
        }
      ],
      "IP": [
        {
          "min": -30,
          "max": -5,
          "sten": 1
        },
        {
          "min": -4,
          "max": -1,
          "sten": 2
        },
        {
          "min": 0,
          "max": 3,
          "sten": 3
        },
        {
          "min": 4,
          "max": 7,
          "sten": 4
        },
        {
          "min": 8,
          "max": 11,
          "sten": 5
        },
        {
          "min": 12,
          "max": 15,
          "sten": 6
        },
        {
          "min": 16,
          "max": 19,
          "sten": 7
        },
        {
          "min": 20,
          "max": 23,
          "sten": 8
        },
        {
          "min": 24,
          "max": 27,
          "sten": 9
        },
        {
          "min": 28,
          "max": 30,
          "sten": 10
        }
      ],
      "IM": [
        {
          "min": -12,
          "max": -7,
          "sten": 1
        },
        {
          "min": -6,
          "max": -5,
          "sten": 2
        },
        {
          "min": -4,
          "max": -3,
          "sten": 3
        },
        {
          "min": -2,
          "max": -1,
          "sten": 4
        },
        {
          "min": 0,
          "max": 1,
          "sten": 5
        },
        {
          "min": 2,
          "max": 4,
          "sten": 6
        },
        {
          "min": 5,
          "max": 6,
          "sten": 7
        },
        {
          "min": 7,
          "max": 8,
          "sten": 8
        },
        {
          "min": 9,
          "max": 10,
          "sten": 9
        },
        {
          "min": 11,
          "max": 12,
          "sten": 10
        }
      ],
      "IZ": [
        {
          "min": -12,
          "max": -6,
          "sten": 1
        },
        {
          "min": -5,
          "max": -4,
          "sten": 2
        },
        {
          "min": -3,
          "max": -2,
          "sten": 3
        },
        {
          "min": -1,
          "max": 0,
          "sten": 4
        },
        {
          "min": 1,
          "max": 2,
          "sten": 5
        },
        {
          "min": 3,
          "max": 4,
          "sten": 6
        },
        {
          "min": 5,
          "max": 6,
          "sten": 7
        },
        {
          "min": 7,
          "max": 8,
          "sten": 8
        },
        {
          "min": 9,
          "max": 10,
          "sten": 9
        },
        {
          "min": 11,
          "max": 12,
          "sten": 10
        }
      ]
    }
  }
}
  $$::jsonb,
  0,
  true
)
on conflict (slug) do update
set title = excluded.title,
    description = excluded.description,
    type = excluded.type,
    json = excluded.json,
    price_rub = excluded.price_rub,
    is_published = excluded.is_published;

commit;
